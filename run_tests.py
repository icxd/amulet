...
def run_test(test_file: str) -> bool:
    ...
    compiler_result = subprocess.run([COMPILER_PATH, test_file, "--nostdlib"], capture_output=True, text=True)
    ...
    clang_result = subprocess.run([
        CLANG_PATH, 
        test_file.replace('.am', '.ll'),
        "-o", test_file.replace('.am', ''),
    ...
    executable_result = subprocess.run([f"./{test_file.replace('.am', '')}"], capture_output=True, text=True)
    ...
    if expected_type not in ['pass', 'fail']:
        print(f"\033[31;1merror: \033[0minvalid expected type `{expected_type}` in {test_file}")
        return False
    ...
    if expected_output_index == -1:
        ...
        return False
    expected_output = expected_output_line[expected_output_index + len("/// output="):].strip()
    if expected_type == 'pass':
        if expected_output != executable_result.stdout.replace('\n', '\\n'):
            print(f"\033[31;1merror: \033[0mexpected output `{expected_output}` but got `{executable_result.stdout}` in {test_file}")
            return False
    elif expected_type == 'fail':
        if expected_output != executable_result.stderr.replace('\n', '\\n'):
            print(f"\033[31;1merror: \033[0mexpected output `{expected_output}` but got `{executable_result.stderr}` in {test_file}")
            return False

    os.remove(test_file.replace('.am', ''))
    os.remove(test_file.replace('.am', '.ll'))
...
def main():
    ...
    test_files = [os.path.join(root, file) for root, _, files in os.walk(TESTS_DIR) for file in files if file.endswith(".am")]
